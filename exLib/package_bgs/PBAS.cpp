...
void PBAS::showResults(bool r)
{
    showOutput = r;
}

PBAS::PBAS(void) : N(20), R_lower(18), Raute_min(2), T_lower(2), T_upper(200),
    R_scale(5), R_incdec(0.05), T_dec(0.05), T_inc(1), firstTime(true), showOutput(false)
{
	//feature vector
	alpha = 7.0;
	beta = 1.0;
...
{
    CvFileStorage* fs = cvOpenFileStorage("./config/PBAS.xml", 0, CV_STORAGE_WRITE);
    ...
    cvWriteInt(fs, "R_incdec", R_incdec);
    cvWriteInt(fs, "T_dec", T_dec);
    cvWriteInt(fs, "T_inc", T_inc);
    cvWriteInt(fs, "alpha", alpha);
    cvWriteInt(fs, "beta", beta);
...
{
    CvFileStorage* fs = cvOpenFileStorage("./config/PBAS.xml", 0, CV_STORAGE_READ);
    ...
    R_incdec = cvReadIntByName(fs, 0, "R_incdec", 0.05);
    T_dec = cvReadIntByName(fs, 0, "T_dec", 0.05);
    T_inc = cvReadIntByName(fs, 0, "T_inc", 1);
    alpha = cvReadIntByName(fs, 0, "alpha", 7.0);
    beta = cvReadIntByName(fs, 0, "beta", 1.0);
...
//bool PBAS::process(cv::Mat* input, cv::Mat* output,cv::Mat &img_bgmodel)
void PBAS::process(cv::Mat const &input_p, cv::Mat &output_p, cv::Mat &img_bgmodel)
{
    cv::Mat tm = input_p;
    cv::Mat* input = &tm;
    cv::Mat* output = &output_p;
    if(width != input->cols)
	{
        width = input->cols;
